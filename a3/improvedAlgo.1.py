...
for entry in origData:
  ...
  if entry[0] > atUser:
    print("processed another user. user per minutes: ", atUser / max(1, ((time.time() - start) / 60)))
    minHashed = np.empty([k, signatureSize], dtype=np.bool)
    cscMovies = movieMatrix.tocsc()
    userEntry = cscMovies[atUser]
    for v in range(k):
      minHashed[v] = userEntry[0, permutations[v]][0, :signatureSize].todense()
    for i in range(bands):
      b = i + 1
      hashedBands[i][atUser] = minHashed[(len(minHashed) / bands) * i : (len(minHashed) / bands) * b]
    for band in range(bands):
      bucketId = hash(str(hashedBands[band][atUser])) % k # int(hashlib.md5(hashedBands[band][atUser]).hexdigest(), 16) % k
      if atUser not in buckets[bucketId]:
        buckets[bucketId].append(atUser)
            ...
            for uId2 in buckets[bucketId]:
              if uId1 < uId2 and str(uId1+uId2) not in alreadyChecked:
                ...
                if similarity >= 0.0001:
                  ...
                  print("pair finder rate per minute is ", pairsWithCheck / ((time.time() - start) / 60))
    atUser += 1
